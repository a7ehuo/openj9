...
static TR::DataType storageTypeForCompactFieldFromSig(const char *fieldSig, bool &isBooleanOut, bool &isCharOut)
{
    switch (fieldSig[0]) {
        case 'Z': // boolean
            isBooleanOut = true;
    ...
    const char *destFieldSignature, int parmIndex)
{
    ...
    if (c == NULL)
        return false;
    if (performTransformation(comp(), "%ssDEBUG symref replaced by %s.%s %s in [%p]\n", OPT_DETAILS, destClass,
            destFieldName, destFieldSignature, node)) {
        ...
        if (type.isIntegral() && compactInstanceFieldEnabled && !node->getOpCode().isIndirect()) {
            if (TR::Options::isAnyVerboseOptionSet())
                TR_VerboseLog::writeLineLocked(TR_Vlog_INFO, "%s: DEBUG symref replaced by %s.%s %s in [%p] %s\n",
                    __FUNCTION__, destClass, destFieldName, destFieldSignature, node, comp()->signature());
        ...
        if (!node->getOpCode().isIndirect()) {
            ...
            if (node->getOpCode().isLoad()) {
                if (processCompactInstanceField) {
                    TR::Node *loadNode
                        = TR::Node::createWithSymRef(comp()->il.opCodeForIndirectCompactLoad(type), 1, symRef);
                    TR::Node *widenedLoadNode
                        = widenIntLoadForCompactFieldIfRequired(loadNode, symRef, isBoolean, isChar);

                    logprintf(comp()->getOption(TR_TraceILGen), comp()->log(),
                        "%s: DEBUG loadNode %p n%dn widenedLoadNode %p n%dn symRef %s\n", __FUNCTION__, loadNode,
                        loadNode->getGlobalIndex(), widenedLoadNode, widenedLoadNode->getGlobalIndex(),
                ...
                if (processCompactInstanceField) {
                    ...
                    logprintf(comp()->getOption(TR_TraceILGen), comp()->log(),
                        "%s: DEBUG oldValueNode %p n%dn newValueNode %p n%dn symRef %s\n", __FUNCTION__, oldValueNode,
                        oldValueNode->getGlobalIndex(), newValueNode, newValueNode->getGlobalIndex(),
                        symRef->getName(comp()->getDebug()));
