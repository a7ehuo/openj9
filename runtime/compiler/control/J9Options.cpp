...
bool J9::Options::feLatePostProcess(void * base, TR::OptionSet * optionSet)
   ...
   // If Control Flow Guard (CFG) is enabled in Windows, do not set HasResumableTrapHandler
   if (J9_ARE_ALL_BITS_SET(javaVM->sigFlags, J9_SIG_WINDOWS_MITIGATION_POLICY_CFG))
      {
      self()->setOption(TR_NoResumableTrapHandler);
      if (self()->showOptionsInEffect())
         {
         TR_VerboseLog::writeLineLocked(TR_Vlog_INFO, "JIT NoResumableTrapHandler=1");
         }
      }
