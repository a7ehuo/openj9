...
bool J9::SymbolReferenceTable::isFieldTypeBool(TR::SymbolReference *symRef)
{
    ...
    dumpOptDetails(comp(), "got field signature as %.*s getName: %s getRecognizedField %d\n", len, fieldSignature,
        symRef->getSymbol()->getName(), symRef->getSymbol()->getRecognizedField());

    if (fieldSignature)
        return isSignatureTypeBool(fieldSignature, len);

    if (symRef->getSymbol()->getName()
        && (strncmp(symRef->getSymbol()->getName(), "java/lang/Boolean.value Z", 25) == 0))
        return true;
...
bool J9::SymbolReferenceTable::isFieldTypeChar(TR::SymbolReference *symRef)
{
    ...
    if (symRef->getSymbol()->getName()
        && (strncmp(symRef->getSymbol()->getName(), "java/lang/Character.value C", 27) == 0))
        return true;
