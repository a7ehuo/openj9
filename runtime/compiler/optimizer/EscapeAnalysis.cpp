...
void TR_EscapeAnalysis::findCandidates()
   ...
   for (_curTree = comp()->getStartTree(); _curTree; _curTree = _curTree->getNextTreeTop())
      ...
      // TODO:  If java.lang.Integer is ever made into a primitive value type class, will
...
bool TR_EscapeAnalysis::tryToZeroInitializeUsingArrayset(Candidate* candidate, TR::TreeTop* precedingTreeTop)
   {
   // TODO:  Investigate whether arrayset can be used for newvalue
...
void TR_EscapeAnalysis::makeNonContiguousLocalAllocation(Candidate *candidate)
   ...
   if (candidate->_node->getOpCodeValue() == TR::newvalue)
      ...
      // If the non-contiguous value type candidate escapes, all fields must be known
...
void TR_EscapeAnalysis::heapifyForColdBlocks(Candidate *candidate)
   ...
   for (info = coldBlockInfoIt.getFirst(); info != NULL; info = coldBlockInfoIt.getNext())
      ...
      TR_ByteCodeInfo bcInfo = candidate->_node->getByteCodeInfo();
      TR::DebugCounter::prependDebugCounter(comp(), TR::DebugCounter::debugCounterName(comp(), "escapeAnalysis/heapification/%s/%s/(%s)/(%d,%d)", comp()->getHotnessName(comp()->getMethodHotness()), comp()->signature(), bcInfo.getCallerIndex() > -1 ? comp()->getInlinedResolvedMethod(bcInfo.getCallerIndex())->signature(trMemory()): comp()->signature(), bcInfo.getCallerIndex(), bcInfo.getByteCodeIndex()), heapAllocationTree);
