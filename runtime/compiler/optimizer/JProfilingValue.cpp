    ...
    TR_AbstractHashTableProfilerInfo *table, bool addNullCheck, bool extendBlocks, bool trace)
{
     ...
     */
    if (isExceptionMetaLoad(profilingValue) || isExceptionMetaLoad(value))
        logprintf(trace, log, "\t\t\tDEBUG isExceptionMetaLoad profilingValue n%dn value n%dn valueChildOfHelperCall %p\n", profilingValue->getGlobalIndex(), value->getGlobalIndex(), valueChildOfHelperCall);
