...
//
// IMPORTANT: Do not rely solely on node pointer equality here. Optimizations
// and duplication/rematerialization may create distinct node instances that
// still load the same symbol. Prefer comparing direct loads by symref.
static bool isSameValueForPreservingStore(TR::Node *candidate, TR::Node *value)
{
    ...
    if (candidate->getOpCode().isLoadDirect()
        && value->getOpCode().isLoadDirect()
        && candidate->getOpCode().hasSymbolReference()
        && value->getOpCode().hasSymbolReference()
...
static TR::Node * findNearestStoreForValue(TR::TreeTop *cursor, TR::Node *value)
{
    for (TR::TreeTop *tt = cursor->getPrevTreeTop();
        ...
        if (ttNode->getOpCode().isStoreDirectOrReg()
...
{
    return store
           && store->getOpCode().isStoreDirect()
           && store->getOpCode().hasSymbolReference()
           && store->getSymbolReference()
           && store->getSymbolReference()->getSymbol()
           && !store->getSymbolReference()->getSymbol()->isMethodMetaData();
}

// Materialize a loadReg that corresponds to a storeReg preservingStore.
// This is used to avoid propagating storeReg as "the value" and to support
// the helper-call/GlRegDeps paths which expect loadReg on fallback.
                ...
                if (isExceptionMetaLoad(value)) {
                    ...
                    if (isStableNonMetaDataStoreDirect(preservingStore)) {
                        ...
                        if (preservingStore && preservingStore->getOpCode().isStoreReg()) {
                            ...
                            dumpOptDetails(comp(), "%s %s: ExceptionMeta preservingStore n%dn value n%dn spillSource n%dn\n",
                                optDetailString(), __FUNCTION__, preservingStore->getGlobalIndex(), value->getGlobalIndex(), spillSource->getGlobalIndex());
                        }
                        ...
                        dumpOptDetails(comp(), "%s %s: ExceptionMeta value n%dn symRef #%d\n", optDetailString(), __FUNCTION__, value->getGlobalIndex(), tmpSymRef ? tmpSymRef->getReferenceNumber() : -1);
                    }
                }
    ...
    if (profilingValue == value && isExceptionMetaLoad(value)) {
        ...
        if (preservingStore) {
            ...
            // spilling/temping logic.
                ...
                logprintf(trace, log,
                    "\t\t\tExceptionMeta preservingStore n%dn ignored (unstable) (value n%dn)!\n",
                    preservingStore->getGlobalIndex(), value->getGlobalIndex());
            }
        }
    }
    ...
    if (valueChildOfHelperCall == NULL) {
        ...
        if (profilingValue->getOpCode().isStoreDirect()
            ...
            logprintf(trace, log, "\t\t\tprofilingValue n%dn is stored on temp slot n%dn #%d\n",
                profilingValue->getGlobalIndex(), storeValue->getNode()->getGlobalIndex(), storedValueSymRef->getReferenceNumber());
        }
