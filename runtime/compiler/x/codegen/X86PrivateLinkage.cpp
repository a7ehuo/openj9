...
void J9::X86::PrivateLinkage::buildInterfaceDispatchUsingLastITable (TR::X86CallSite &site, int32_t numIPicSlots, TR::X86PICSlot &lastPicSlot, TR::Instruction *&slotPatchInstruction, TR::LabelSymbol *doneLabel, TR::LabelSymbol *lookupDispatchSnippetLabel, TR_OpaqueClassBlock *declaringClass, uintptr_t itableIndex )
   ...
   */
   ...
   if (trace)
      ...
      traceMsg(comp(), "%s: use32BitInterfacePointers %d itableIndex %u declaringClass 0x%p %s\n", __FUNCTION__, use32BitInterfacePointers, itableIndex, declaringClass, classSig);
      }

   static char *debugLoopITable = feGetEnv("TR_DebugLoopITable");
   if (!debugLoopITable)
      {
      ...
      if (!debugCheckTwoITableEntries)
         ...
         {
         generateLongLabelInstruction(TR::InstOpCode::JE4, callNode, gotoLastITableDispatchLabel, cg()); // PICBuilder needs this to have a 4-byte offset

        //----------------------------------------------
         // Check Second J9ITable
   ...
   if (debugLoopITable)
      {
      generateLabelInstruction(TR::InstOpCode::label, callNode, iTableLoopLabel, cg());
      ...
      generateLabelInstruction(TR::InstOpCode::JE4, callNode, gotoLastITableDispatchLabel, cg());
...
#if 0 // Loop through itable
   if (debugLoopITable)
      {
      TR::LabelSymbol *iTableIterateLoopLabel = generateLabelSymbol(cg());
      ...
      generateLabelInstruction(TR::InstOpCode::label, callNode, iTableIterateLoopLabel, cg());

      // J9Class.itable == NULL?
      generateRegRegInstruction(TR::InstOpCode::TESTRegReg(), callNode, scratchReg, scratchReg, cg());
      generateLabelInstruction(TR::InstOpCode::JE4, callNode, lookupDispatchSnippetLabel, cg());

      // if J9ITable.interfaceClass == declaringClass ?
      //auto interfaceMR = generateX86MemoryReference(scratchReg, offsetof(J9ITable, interfaceClass), cg());
      //generateMemImmInstruction(TR::InstOpCode::CMPMemImms(), callNode, interfaceMR, (intptr_t)declaringClass, cg());
      if (use32BitInterfacePointers)
         {
         // The field is 8 bytes, but only 4 matter
         generateMemImmInstruction(TR::InstOpCode::CMP4MemImm4,
      ...
      // scratchReg = itable->next
      generateRegMemInstruction(TR::InstOpCode::LRegMem(), callNode, scratchReg, generateX86MemoryReference(scratchReg, offsetof(J9ITable, next), cg()), cg());
      generateLabelInstruction(TR::InstOpCode::JNE4, callNode, iTableIterateLoopLabel, cg());
