...
TR_ResolvedJ9Method::isFieldQType(int32_t cpIndex)
   ...
   bool isQType = vmThread->javaVM->internalVMFunctions->isNameOrSignatureQtype(signature);
   if (isQType)
      printf("\n\nisFieldQType true %.*s:%.*s\n",
         J9UTF8_LENGTH(J9ROMNAMEANDSIGNATURE_NAME(nameAndSignature)), utf8Data(J9ROMNAMEANDSIGNATURE_NAME(nameAndSignature)),
         J9UTF8_LENGTH(J9ROMNAMEANDSIGNATURE_SIGNATURE(nameAndSignature)), utf8Data(J9ROMNAMEANDSIGNATURE_SIGNATURE(nameAndSignature)));
...
TR_ResolvedJ9Method::isFieldFlattened(TR::Compilation *comp, int32_t cpIndex, bool isStatic)
   ...
   TR::VMAccessCriticalSection isFieldFlattened(fej9());

   TR_ASSERT_FATAL(fieldShape && containingClass, "fieldShape %p containingClass %p cannot be NULL", fieldShape, containingClass);
   bool isFlattened = vmThread->javaVM->internalVMFunctions->isFlattenableFieldFlattened(reinterpret_cast<J9Class *>(containingClass), fieldShape);
   if (isFlattened)
      {
      ...
      printf("%s: isFlattend true owningMethod %.*s%.*s field %.*s:%.*s for compilation %s\n",
               __FUNCTION__,
               nameLength(), nameChars(), signatureLength(), signatureChars(),
               fieldNameLength, fieldName, sigLength, fieldSignature, comp->signature());
      }
